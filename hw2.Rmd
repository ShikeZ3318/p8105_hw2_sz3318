...
*  The average in Season1 is 6.125, and the avrage of season 5 is 6.25
```{r}
# View bakers dataset
glimpse(viewers)
summary(viewers)
...
```{r}
# Filter for Series 1 to 10 and Episode 1, then calculate the mean viewership
episode_1_avg <- viewers_clean %>%
  filter(series >= 1 & series <= 10 & Episode == 1) %>%
  summarise(avg_viewership = mean(series, na.rm = TRUE))

episode_1_avg

# Filter for Series 1 to 10 and Episode 5, then calculate the mean viewership
episode_5_avg <- viewers_clean %>%
  filter(series >= 1 & series <= 10 & Episode == 5) %>%
  summarise(avg_viewership = mean(series, na.rm = TRUE))
